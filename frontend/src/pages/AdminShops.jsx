    ...
    <>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-gray-800 p-5 sm:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 lg:sticky lg:top-24 transition-colors">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <button onClick={handleSave} className="w-full sm:flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-100 dark:shadow-none hover:bg-blue-700 transition-all">
                    {editingId ? "Update Shop" : "Save Shop"}
                  </button>
                  {editingId && (
                    <button onClick={resetForm} className="w-full sm:px-4 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-gray-600 transition-all">
                      Cancel
                    </button>
                  )}
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-colors">
              <div className="bg-gray-50 dark:bg-gray-900/50 px-4 sm:px-8 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center transition-colors">
                ...
                <span className="bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-400 px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-widest transition-all">
                    {filteredShops.length} <span className="hidden sm:inline">Locations</span><span className="sm:hidden">Locs</span>
                </span>
              </div>
              <div className="overflow-x-auto max-h-[calc(100vh-300px)] overflow-y-auto custom-scrollbar">
                {filteredShops.length === 0 ? (
                    ...
                    <table className="w-full relative border-collapse min-w-[500px] md:min-w-full">
                    <thead className="bg-gray-50/90 dark:bg-gray-900/90 transition-colors sticky top-0 z-10 backdrop-blur-sm">
                        <tr>
                        <th className="px-4 sm:px-8 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">Shop Details</th>
                    ...
                    <tbody className="divide-y divide-gray-50 dark:divide-gray-700 bg-white dark:bg-gray-800 transition-colors">
                        ...
                        <tr key={shop.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-900/50 transition-colors">
                            ...
                            <div className="font-bold text-gray-900 dark:text-white text-base sm:text-lg transition-colors">{shop.name}</div>
                            <div className="flex items-center gap-1 text-gray-500 dark:text-gray-400 text-xs sm:text-sm mt-1 transition-colors">
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-2">
                                <button onClick={() => navigate(`/admin/shops/${shop.id}/stock`)} className="w-full sm:w-auto bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 p-2 rounded-lg hover:bg-green-600 hover:text-white transition-all flex items-center justify-center gap-1 font-bold text-xs">
                                <Box size={14} /> Stock
                                </button>
