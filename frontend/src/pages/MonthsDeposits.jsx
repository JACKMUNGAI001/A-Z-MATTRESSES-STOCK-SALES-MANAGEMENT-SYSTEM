...
export default function MonthsDeposits() {
  ...
  useEffect(() => {
    const fetchDeposits = async () => {
      try {
        const response = await api.get('/deposits/month');
        setDeposits(response.data);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching this month's deposits:", error);
        setLoading(false);
      }
    };
    fetchDeposits();
  ...
  return (
    <>
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight transition-colors">Monthly Collection Summary</h1>
            <p className="text-gray-500 dark:text-gray-400 font-medium transition-colors">Tracking all installment payments received during the current month</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-colors">
          <div className="bg-indigo-50 dark:bg-indigo-900/30 px-8 py-4 border-b border-indigo-100 dark:border-indigo-900/50 flex justify-between items-center transition-colors">
            <h2 className="text-lg font-bold text-indigo-800 dark:text-indigo-400 flex items-center gap-2 transition-colors">
              ...
              Monthly Collection Log
            </h2>
            <div className="flex items-center gap-4">
               <div className="text-right">
                  <div className="text-[10px] text-indigo-400 dark:text-indigo-500 font-black uppercase tracking-widest transition-colors">Month Total</div>
                  <div className="text-lg font-black text-indigo-800 dark:text-indigo-400 transition-colors">
                    {formatCurrency(filteredDeposits.reduce((acc, p) => {
                      const now = new Date();
                      const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
                      startOfMonth.setHours(0,0,0,0);
                      const amountPaidThisMonth = p.payments
                        .filter(pay => new Date(pay.paid_on) >= startOfMonth)
                        .reduce((sum, pay) => sum + pay.amount, 0);
                      return acc + amountPaidThisMonth;
                    }, 0))}
                  </div>
               </div>
          ...
          <div className="overflow-x-auto max-h-[calc(100vh-300px)] overflow-y-auto custom-scrollbar">
            {loading ? (
              ...
              <div className="p-20 text-center border-t border-gray-100 dark:border-gray-700 transition-colors">
                ...
                <p className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-widest text-sm transition-colors">{searchQuery ? `No matches found for "${searchQuery}"` : 'No deposit payments recorded for this month.'}</p>
              </div>
            ) : (
              <table className="w-full relative border-collapse">
                <thead className="bg-gray-50/90 dark:bg-gray-900/90 transition-colors sticky top-0 z-10 backdrop-blur-sm">
                  <tr>
                    ...
                    <th className="px-8 py-4 text-right text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">Paid this Month</th>
                  ...
                  {filteredDeposits.map((p, idx) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition-colors">
                        ...
                        <td className="px-8 py-4 text-right font-black text-indigo-600 dark:text-indigo-400 text-lg transition-colors">{formatCurrency(amountPaidThisMonth)}</td>
