    ...
    <>
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium transition-colors">Tracking all installment payments received during the current year</p>
               ...
               <div className="text-right">
                  ...
                  <div className="text-lg font-black text-indigo-800 dark:text-indigo-400 transition-colors">
                    {formatCurrency(filteredDeposits.reduce((acc, p) => {
                      ...
                      const amountPaidThisYear = p.payments
                        .filter(pay => new Date(pay.paid_on) >= startOfYear)
                        .reduce((sum, pay) => sum + pay.amount, 0);
                      return acc + amountPaidThisYear;
                    }, 0))}
                  </div>
               </div>
              ...
              <table className="w-full relative border-collapse">
                <thead className="bg-gray-50/90 dark:bg-gray-900/90 transition-colors sticky top-0 z-10 backdrop-blur-sm">
                  <tr>
                    ...
                    <th className="px-8 py-4 text-right text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">Paid this Year</th>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-700 bg-white dark:bg-gray-800 transition-colors">
                  {filteredDeposits.map((p, idx) => {
                    ...
                    // Calculate amount paid this year
                    const now = new Date();
                    const startOfYear = new Date(now.getFullYear(), 0, 1);
                    startOfYear.setHours(0,0,0,0);
                    ...
                    return (
                      <tr key={idx} className="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition-colors">
                        ...
                        <td className="px-8 py-4 text-right font-black text-indigo-600 dark:text-indigo-400 text-lg transition-colors">{formatCurrency(amountPaidThisYear)}</td>
