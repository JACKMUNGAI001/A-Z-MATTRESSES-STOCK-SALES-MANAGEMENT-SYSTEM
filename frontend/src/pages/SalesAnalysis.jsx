...
import { BarChart3, Store, Calendar, Clock, Filter, Package, TrendingUp } from "lucide-react";
    ...
    <div className="p-6 md:p-10 bg-gray-50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">Identify top-performing products and shop trends</p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <select
              className="w-full border-2 border-gray-100 p-3 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none font-bold transition-all"
              value={period}
      ...
      {loading ? (
        <div className="mt-20 text-center">
          ...
          <p className="text-gray-500 font-bold uppercase tracking-widest text-sm animate-pulse">Analyzing sales data...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
            <div className="bg-gray-50 px-8 py-6 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="bg-blue-100 text-blue-700 text-[10px] font-black px-3 py-1 rounded-full uppercase">Global Ranking</span>
            </div>
                    ...
                    analysis.overall.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50/50 transition-colors group">
                        <td className="px-4 py-4">
                          <span className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{item.product_name}</span>
                        </td>
                        <td className="px-4 py-4 text-center">
                          <span className="inline-flex items-center justify-center bg-gray-100 text-gray-700 font-black px-3 py-1 rounded-lg text-sm">
                            {item.total_qty}
                          </span>
                        </td>
          ...
          <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
            <div className="bg-gray-50 px-8 py-6 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="bg-indigo-100 text-indigo-700 text-[10px] font-black px-3 py-1 rounded-full uppercase">Location Breakdown</span>
            </div>
            <div className="p-4 overflow-x-auto">
              <table className="w-full">
                <thead>
                ...
                <tbody className="divide-y divide-gray-50">
                  {analysis.by_shop.length > 0 ? (
                    analysis.by_shop.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50/50 transition-colors group">
                        <td className="px-4 py-4 font-medium text-gray-500">{item.shop_name}</td>
                        <td className="px-4 py-4 font-bold text-gray-900">{item.product_name}</td>
                        <td className="px-4 py-4 text-center">
                          <span className="font-bold text-gray-700">{item.total_qty}</span>
                        </td>
                        <td className="px-4 py-4 text-right font-black text-gray-900">
                          {formatCurrency(item.total_revenue)}
                        </td>
